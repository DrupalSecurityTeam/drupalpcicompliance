...
For many smaller eCommerce stores, selecting a payment method that places them in type C or D can be cost prohibitive. However, achieving type A is not always possible given a businesses features and requirements, which drives the expansion or reduction of the CDE and ultimately determines the validation type.
...
A HPP approach is where a user is redirected from a Drupal site to a third-party site in order to enter their payment details. If the transaction is successful, the customer is redirected back to the Drupal site with an authorization code that validates the payment was accepted.
...
A direct post approach is where the customer remains on the website, but the payment form is submitted directly from the customer’s browser to the payment gateway and a response is sent directly back to the customer with an authentication code that is then submitted to the Drupal application.
